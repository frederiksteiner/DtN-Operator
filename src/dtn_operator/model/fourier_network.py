...
def batch_conv_1d(x, batch_kernel):
    """Batch convolution."""
...
class FourierNetwork(nn.Module):
    ...
    def __init__(self, size_in, num_of_modes, kernel_in, kernel_modes, num_of_layers):
        ...
        for _ in range(num_of_layers):
            ...
            self.green.append(
                KernelNetwork(self.kernel_in, self.kernel_modes, self.outsize, self.size_in, self.modes))

            self.lin_layer_real.append(
                LinearKernelNetwork(self.kernel_in, self.kernel_modes, self.size_in, self.size_in, self.modes))
    ...
    def forward(self, x, y):
        ...
        for i in range(self.numOfLayers - 1):
            ...
            shape = x1.shape
            x2 = batch_conv_1d(x, torch.unsqueeze(self.linLayReal[i](y), -1))
            x = x1 + x2.view(shape)
            # pylint: disable=not-callable
            x = F.gelu(x)

        weights = self.green[self.numOfLayers - 1](y)
        x1 = self.fourreal[self.numOfLayers - 1](x, weights)
        x2 = batch_conv_1d(x, torch.unsqueeze(self.linLayReal[self.numOfLayers - 1](y), -1))
        x = x1 + x2.view(shape)
